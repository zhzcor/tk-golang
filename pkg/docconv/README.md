# docconv
[![GoDoc](https://godoc.org/gserver/pkg/docconv?status.svg)](https://godoc.org/gserver/pkg/docconv)
...
**Note for returning users:** the Go code path for this pkg been moved to `gserver/pkg/docconv`.  Follow the installation instructions to checkout a version of the code in the correct place.
...
To add image support to the `docconv` library you first need to install and build https://github.com/otiai10/gosseract.  Now you can add `-tags ocr` to any `go` command when building/fetching `docconv` to include support for processing images:
    ...
    $ go get -tags ocr gserver/pkg/docconv/...
