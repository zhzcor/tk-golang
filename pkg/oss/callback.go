...
type PolicyToken struct {
	...
	CallbackParam CallbackParam `json:"callback_param"`
}
...
func GetPolicyToken(dir string) (PolicyToken, error) {
	...
	h := hmac.New(func() hash.Hash { return sha1.New() }, []byte(aliyunConfig.AccessKeySecret))
	io.WriteString(h, debyte)
	signedStr := base64.StdEncoding.EncodeToString(h.Sum(nil))

	var callbackParam CallbackParam
	callbackParam.CallbackUrl = aliyunConfig.OssCallback
	callbackParam.CallbackBody = "filename=${object}&size=${size}&mimeType=${mimeType}&height=${imageInfo.height}&width=${imageInfo.width}&admin_id=1"
	...
	policyToken.AccessKeyId = aliyunConfig.AccessKeyId
	policyToken.Host = aliyunConfig.OssHost
	policyToken.Expire = expire_end
	policyToken.Signature = string(signedStr)
	policyToken.Directory = dir
	policyToken.Policy = string(debyte)
	policyToken.Callback = string(callbackBase64)
	policyToken.CallbackParam = callbackParam
