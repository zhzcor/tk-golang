...
func (cq *CollectionQuery) QueryQuestion() *TkQuestionQuery {
	query := &TkQuestionQuery{config: cq.config}
...
func (cq *CollectionQuery) Clone() *CollectionQuery {
	...
	return &CollectionQuery{
		...
		withQuestion: cq.withQuestion.Clone(),
...
func (cq *CollectionQuery) WithQuestion(opts ...func(*TkQuestionQuery)) *CollectionQuery {
	query := &TkQuestionQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withQuestion = query
	return cq
}
...
func (cq *CollectionQuery) sqlAll(ctx context.Context) ([]*Collection, error) {
	...
	if query := cq.withQuestion; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "value_id" returned %v`, n.ID)
			}
			for i := range nodes {
