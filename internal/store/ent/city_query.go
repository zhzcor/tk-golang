...
func (cq *CityQuery) QueryKcClass() *KcClassQuery {
	query := &KcClassQuery{config: cq.config}
...
func (cq *CityQuery) QueryCourse() *KcCourseQuery {
	query := &KcCourseQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryUserCity chains the current query on the "user_city" edge.
func (cq *CityQuery) QueryUserCity() *UserQuery {
...
func (cq *CityQuery) WithKcClass(opts ...func(*KcClassQuery)) *CityQuery {
	query := &KcClassQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withKcClass = query
	return cq
}
...
func (cq *CityQuery) WithCourse(opts ...func(*KcCourseQuery)) *CityQuery {
	query := &KcCourseQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
// WithUserCity tells the query-builder to eager-load the nodes that are connected to
// the "user_city" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CityQuery) WithUserCity(opts ...func(*UserQuery)) *CityQuery {
	query := &UserQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CityQuery) sqlAll(ctx context.Context) ([]*City, error) {
	...
	if query := cq.withKcClass; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*City)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.KcClass = []*KcClass{}
		}
		query.Where(predicate.KcClass(func(s *sql.Selector) {
			s.Where(sql.InValues(city.KcClassColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.KcClass = append(node.Edges.KcClass, n)
		}
	}
	...
	if query := cq.withCourse; query != nil {
		...
		query.Where(predicate.KcCourse(func(s *sql.Selector) {
			s.Where(sql.InValues(city.CourseColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
	...
	if query := cq.withUserCity; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*City)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.UserCity = []*User{}
		}
		query.Where(predicate.User(func(s *sql.Selector) {
			s.Where(sql.InValues(city.UserCityColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
