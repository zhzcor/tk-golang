...
func (mq *MessageQuery) QueryMessageType() *MessageTypeQuery {
	query := &MessageTypeQuery{config: mq.config}
...
func (mq *MessageQuery) QueryAdmin() *AdminQuery {
	query := &AdminQuery{config: mq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (mq *MessageQuery) QueryAttachment() *AttachmentQuery {
	query := &AttachmentQuery{config: mq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (mq *MessageQuery) QueryCourse() *KcCourseQuery {
...
func (mq *MessageQuery) QueryClass() *KcClassQuery {
...
func (mq *MessageQuery) Clone() *MessageQuery {
	...
	return &MessageQuery{
		...
		withAdmin:       mq.withAdmin.Clone(),
		withAttachment:  mq.withAttachment.Clone(),
...
func (mq *MessageQuery) WithAdmin(opts ...func(*AdminQuery)) *MessageQuery {
	query := &AdminQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MessageQuery) WithAttachment(opts ...func(*AttachmentQuery)) *MessageQuery {
	query := &AttachmentQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
	mq.withAttachment = query
	return mq
}
...
func (mq *MessageQuery) WithCourse(opts ...func(*KcCourseQuery)) *MessageQuery {
	query := &KcCourseQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MessageQuery) WithClass(opts ...func(*KcClassQuery)) *MessageQuery {
	query := &KcClassQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MessageQuery) sqlAll(ctx context.Context) ([]*Message, error) {
	...
	if query := mq.withMessageType; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "message_type_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.MessageType = n
			}
		}
	}

	if query := mq.withAdmin; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Message)
		for i := range nodes {
	...
	if query := mq.withAttachment; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Message)
		for i := range nodes {
	...
	if query := mq.withUsers; query != nil {
		...
		ids := make(map[int]*Message, len(nodes))
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(message.UsersPrimaryKey[1], fks...))
			},
